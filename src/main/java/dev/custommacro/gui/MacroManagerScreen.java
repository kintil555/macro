...
public class MacroManagerScreen extends Screen {
    ...
    private int     editIndex = -1;   // -1 = new entry
    ...
    private void buildRowButtons() {
        int visible = SCROLL_AREA_H / ROW_H;
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("▼"), btn -> {
            int maxScroll = Math.max(0, entries.size() - SCROLL_AREA_H / ROW_H);
    ...
    private void rebuildList() {
        ...
        buildRowButtons();
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("custommacro.button.add"), btn -> openEdit(-1)
        ).dimensions(px + PANEL_W - 130, py + PANEL_H - 30, 120, 20).build());
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("custommacro.button.close"), btn -> close()
        ).dimensions(px + 10, py + PANEL_H - 30, 70, 20).build());
    }
    ...
    private void initEditView() {
        ...
        String keyLabel = (editIndex >= 0) ? keyName(pendingKeyCode) : "[ Click to bind key ]";
        ...
        addDrawableChild(ButtonWidget.builder(
                Text.translatable("custommacro.button.save"), btn -> saveEdit()
    ...
    private void saveEdit() {
        ...
        if (editIndex < 0 || editIndex >= entries.size()) entries.add(entry);
        else                                               entries.set(editIndex, entry);
    ...
    // ── Key input (1.21.9+ signature: keyPressed(KeyInput)) ───────────────────
    @Override
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Draw border manually (4 rectangles) since renderOutline name may vary
        context.fill(px,               py,               px + PANEL_W, py + 1,          0xFFCC0000); // top
        context.fill(px,               py + PANEL_H - 1, px + PANEL_W, py + PANEL_H,    0xFFCC0000); // bottom
        context.fill(px,               py,               px + 1,       py + PANEL_H,    0xFFCC0000); // left
        context.fill(px + PANEL_W - 1, py,               px + PANEL_W, py + PANEL_H,    0xFFCC0000); // right
        ...
        if (!editing) renderListView(context);
        else          renderEditView(context);
    ...
    private void renderListView(DrawContext context) {
        ...
        if (entries.isEmpty()) {
            ...
            for (int i = 0; i < visible; i++) {
                ...
                context.fill(px + 8, rowY, px + PANEL_W - 170, rowY + ROW_H - 2,
                        (i % 2 == 0) ? 0x22FFFFFF : 0x11FFFFFF);

                context.drawTextWithShadow(textRenderer, Text.literal(truncate(e.getName(),   16)), px + 12,  rowY + 5, 0xFFFFFF);
                context.drawTextWithShadow(textRenderer, Text.literal("[" + keyName(e.getKeyCode()) + "]"),   px + 120, rowY + 5, 0xFFDD44);
                context.drawTextWithShadow(textRenderer, Text.literal(truncate(e.getAction(), 18)), px + 175, rowY + 5, 0xAAFFAA);
            }
        }
        ...
        if (entries.size() > SCROLL_AREA_H / ROW_H) {
            String info = (scrollOffset + 1) + "-"
                    ...
                    + " / " + entries.size();
            context.drawTextWithShadow(textRenderer, Text.literal(info), px + 12, py + PANEL_H - 25, 0x888888);
        }
    }

    private void renderEditView(DrawContext context) {
        String title = (editIndex < 0) ? "Add New Macro" : "Edit Macro";
        context.drawCenteredTextWithShadow(textRenderer, Text.literal(title), px + PANEL_W / 2, py + 28, 0xFFDD88);

        context.drawTextWithShadow(textRenderer, Text.translatable("custommacro.label.name"),   px + 12, py + 63,  0xCCCCCC);
        context.drawTextWithShadow(textRenderer, Text.translatable("custommacro.label.action"), px + 12, py + 93,  0xCCCCCC);
        context.drawTextWithShadow(textRenderer, Text.translatable("custommacro.label.key"),    px + 12, py + 123, 0xCCCCCC);
        ...
        if (nameField   != null && nameField.getText().isEmpty())
            context.drawTextWithShadow(textRenderer, Text.literal("← required"), px + 315, py + 63, 0xFF4444);
        if (actionField != null && actionField.getText().isEmpty())
            context.drawTextWithShadow(textRenderer, Text.literal("← required"), px + 315, py + 93, 0xFF4444);
    }
